# for extracting stats sumary from STAN output
## require rstan  ##
library(rstan)
# for Pailin data
FittedData <- function(posIndx, datafiles) {
  ...
  ex <- extract(modfit, pars = c("y_pred", "mod_predSplitCT"))
  ...
  obTnp <- bigcounts[[posIndx]]$NPPARA
  obT <- ObTimes[1:obTnp]
  
  outdataframe <- data.frame(obtime = c(0, obT[2:obTnp]), obdat = bigcounts[[posIndx]]$y, median = y_predMed, 
...
#for Wang Pha data
FittedDataWP <- function(posIndx,datafiles) {
  ObTimes <- c(1, 4, 8, 12, 18, 24, 30, 36, 42, 48, 54, 60, 66, 72, 78,
               84, 90, 96, 102, 108, 114, 120, 126, 132, 138, 144, 150, 156, 162, 168, 174)
  ...
  mod_predSplitCTMed <- apply(ex$mod_predSplitCT, 2, median)
  mod_predSplitCTCI <- apply(ex$mod_predSplitCT, 2, quantile, probs = c(0.025, 0.975))
  obTnp <- bigcountsWP[[posIndx]]$NPPARA
  obT <- ObTimes[1:obTnp]
  
  outdataframe <- data.frame(obtime = c(0, obT[2:obTnp]), obdat = bigcountsWP[[posIndx]]$y, median = y_predMed,
                             below = y_predCI[1,], above = y_predCI[2,], mediansplit = mod_predSplitCTMed[obT],
                             belowsplit = mod_predSplitCTCI[1, obT], abovesplit = mod_predSplitCTCI[2, obT])
